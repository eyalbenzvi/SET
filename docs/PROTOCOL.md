...
| Method | Path                  | Purpose                                                                                  |
| ------ | --------------------- | ---------------------------------------------------------------------------------------- |
| `GET`  | `/health`             | Liveness. `{"ok":true,"protocol":1}`.                                                    |
| `POST` | `/api/rooms`          | Create a room. `201 {"code":"ABC234","protocol":1}`.                                     |
...
| Frame                                                          | Notes                                                                        |
| -------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| `{"t":"hello","v":1,"name":"Maya"}`                            | First frame. Required before anything else.                                  |
| `{"t":"hello","v":1,"name":"Maya","playerId":"…","token":"…"}` | Resume a seat. Both credentials required together, or the frame is refused.  |
...
| -------------------------------------------------------- | --------- | -------------------------------------------------------------------------------- |
| `{"t":"welcome","v":1,"you":{playerId,token},"state":…}` | Private   | The only frame that ever contains this player's token.                           |
...
`noSetRejected`, `rematchWanted`, `gameOver`.
...
never does: being beaten to a set is not a mistake.
