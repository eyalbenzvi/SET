...
| Method | Path                  | Purpose                                                                                  |
| ------ | --------------------- | ---------------------------------------------------------------------------------------- |
| `GET`  | `/health`             | Liveness. `{"ok":true,"protocol":3}`.                                                    |
| `POST` | `/api/rooms`          | Create a room. `201 {"code":"ABC234","protocol":3}`.                                     |
...
| Frame                                                          | Notes                                                                        |
| -------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| `{"t":"hello","v":3,"name":"Maya"}`                            | First frame. Required before anything else.                                  |
| `{"t":"hello","v":3,"name":"Maya","playerId":"…","token":"…"}` | Resume a seat. Both credentials required together, or the frame is refused.  |
...
| -------------------------------------------------------- | --------- | -------------------------------------------------------------------------------- |
| `{"t":"welcome","v":3,"you":{playerId,token},"state":…}` | Private   | The only frame that ever contains this player's token.                           |
...
```jsonc
{
  ...
  "autoDealAt": 0            // when three cards land on a board with no SET; 0 while one is findable
}
```
...
`hostChanged`, `gameStarted`, `setFound`, `invalidClaim`, `noSetOnBoard`,
`cardsAdded`, `dealVote`, `dealLapsed`, `hintUsed`, `rematchWanted`,
`gameOver`.
...
carries `reason`: `auto` when the server resolved a board with no SET in it,
`agreed` when the whole table asked for them.

`noSetOnBoard` is broadcast the moment the server proves the board has no SET,
which is after every board change and needs no client to ask. It names no player,
because nobody called it. Its `dealsAt` is the server clock at which three cards
will land — `PublicState.autoDealAt` carries the same deadline for clients that
join or reconnect mid-pause — and is `0` when there is nothing left to deal, in
which case `gameOver` follows in the same batch.
...
Only `not_a_set` applies the 2-second cooldown. `board_changed` never does: being
beaten to a set is not a mistake. Nor does `no_set_on_board`, which means the
claim was made in the pause between a dead board being announced and its
replacement cards arriving — there was no correct claim to make. Asking for cards
or for a hint never costs a cooldown either, and both are allowed while one is
running — they are requests, not moves on the board.
